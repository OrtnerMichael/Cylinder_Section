...
def el30(x, kc, p):
    ...
    r = np.abs(p)
    h = 1. + hh
    if (e<.1 and z<.1 and t<1 and r<1):
...
def el3v(x, kc, p):
    ...
    r = np.abs(p)
    h = 1. + hh
    mask2 = (e<.1) * (z<.1) * (t<1) * (r<1)
    if any(mask2):
        ra, rb, rr = np.zeros((3, ND-1, np.sum(mask2)))
        px, pmx, tx, hhx, hx, xx = p[mask2], pm[mask2], t[mask2], hh[mask2], h[mask2], x[mask2]
        for k in range(2, ND+1):
            km2=int(k-2)
            rb[km2]=0.5/k
            ra[km2]=1.0-rb[km2]
    ...
    mask2x = ~mask2
    p, pm, t, hh, h, x = p[mask2x], pm[mask2x], t[mask2x], hh[mask2x], h[mask2x], x[mask2x]
    ...
    if np.any(box):
        ...
        if np.any(mask8):
            ...
            z[mask8] = (hh[mask8]*r[mask8]*f[mask8] - g[mask8]*ap[mask8] + ye[mask8]) / t[mask8]
    ...
    while np.any(mask10):
        ...
        if np.any(bo10x):
            ...
            if np.any(bo10x_bk):
                ...
                ye[bo10x_bk] = ye[bo10x_bk] * (h[bo10x_bk]+1/h[bo10x_bk]) + de[bo10x_bk] * (
                        1+r[bo10x_bk])
